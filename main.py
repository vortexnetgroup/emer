...
@discord.app_commands.describe(zipcode="The 5-digit ZIP code", time="Timeframe for the map")
@discord.app_commands.choices(time=[
    discord.app_commands.Choice(name="Latest", value="latest"),
    discord.app_commands.Choice(name="15 Minutes Ago", value="15"),
    discord.app_commands.Choice(name="30 Minutes Ago", value="30"),
    discord.app_commands.Choice(name="1 Hour Ago", value="60"),
])
async def iemradmap(interaction: discord.Interaction, zipcode: str, time: str):
    ...
    params = {
        ...
        'width': 640,
        'height': 480,
        'title': f"NEXRAD Radar for {city}, {state}"
    }
    ...
    if time != "latest":
        ...
        past_time = now - delta
        params['ts'] = past_time.strftime('%Y%m%d%H%M')

    # The base URL for the radmap
    radmap_base_url = "https://mesonet.agron.iastate.edu/GIS/radmap.php"
    
    # Create the full URL with encoded parameters
    image_url = f"{radmap_base_url}?{urllib.parse.urlencode(params)}"

    embed = discord.Embed(
        title=f"IEM Radar Map for {city}, {state}",
        color=discord.Color(0xffffff)
    )
    embed.set_image(url=image_url)
    embed.set_footer(text="Map generated by IEM")
